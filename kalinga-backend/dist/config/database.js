...
const isMock = env_js_1.config.DATABASE_URL.toLowerCase() === 'mock';
// Define the directory for mock database JSON files
const MOCK_DB_DIR = path_1.default.join(process.cwd(), 'data', 'mock_db');
// Helper to initialize and load mock data
function loadMockData(filename, defaultData = []) {
    if (!fs_1.default.existsSync(MOCK_DB_DIR)) {
        fs_1.default.mkdirSync(MOCK_DB_DIR, { recursive: true });
    }
    const filePath = path_1.default.join(MOCK_DB_DIR, filename);
    if (!fs_1.default.existsSync(filePath)) {
        fs_1.default.writeFileSync(filePath, JSON.stringify(defaultData, null, 2), 'utf-8');
        return defaultData;
    }
    try {
...
class MockPool {
    ...
    async query(sql, params = []) {
        ...
        const patients = loadMockData('patients.json');
        const packets = loadMockData('triage_packets.json');
        const syncEvents = loadMockData('sync_events.json');
        ...
        if (normalizedSql.includes('INSERT INTO patients')) {
            const [id, midwife_id, full_name, philhealth_id, age, lmp, estimated_due_date, gravida, para, risk_factors, barangay, municipality, province, contact_number] = params;
        ...
        if (normalizedSql.includes('INSERT INTO triage_packets')) {
            const [id, patient_id, midwife_id, systolic_bp, diastolic_bp, heart_rate, gestational_age_weeks, bmi, protein_urine, symptoms, frame_base64, frame_thumbnail_b64, ai_prediction_normal, ai_prediction_abnormal, ai_prediction_inconcl, ai_inference_time_ms, risk_score, triage_level, client_captured_at, barangay_station, gps_latitude, gps_longitude] = params;
